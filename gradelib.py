...
def shell_script(script, terminate_match=None):
    ...
    def setup_call_on_line(runner):
        ...
        def handle_output(output):
            ...
            # shell 在执行文件中的命令时，不会打印 $ 符号，导致context.buf 中包含执行的命令
            if context.executed < context.n:
                scriptBytes = bytearray(script[context.executed] + "\n", "utf-8")
                if context.buf.startswith(scriptBytes):
                    context.buf = context.buf[len(scriptBytes):]
                    context.executed += 1
